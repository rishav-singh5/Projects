...
root =  Tk()
root.title('DSA PROJECT Algorithm Visualizer')
...
#A STAR VISUALIZER
def open_astar():
    subprocess.Popen(['python', 'astar_visualizer.py'])
...
Button(options_frame, text="Current Array", command=Generate_array, bg='blue', height=5).grid(row=1, column=3, padx=10,
                                                                                              pady=10)
Button(options_frame, text="Run A* Pathfinding", command=open_astar, bg='purple', fg='white', height=2).grid(row=2, column=3, padx=5, pady=5)
